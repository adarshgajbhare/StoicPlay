...
function FeedCard({ feedName, onDelete }) {
  const imageUrl =
    localStorage.getItem(`${feedName}-image`) || "default-image.jpg";

  return (
    <div
      className="bg-cover  bg-center h-48 rounded-md relative "
      style={{ backgroundImage: `url(${imageUrl})` }}
    >
      <Link to={`/feed/${feedName}`}>
        <div
          className="absolute inset-0 bg-black bg-opacity-50 rounded-md  flex flex-col items-center
